...
`{receivables_model, payables_model, ...}`; FX has a single target and is packaged as
`{model, feature_columns, log_transform, hyperparameters}`).
...
| FX foreign revenue | `cross_border_inflows`, `txn_count`, `n_countries` (log-transformed) | `revenue × foreign_revenue_pct` | `cross_border_inflows / foreign_revenue` |
...
| FX foreign revenue | ~0.38 | Raw-value single-feature fit was negative across all 25 combinations - only 17 of 43 company-years have a usable label, and `share_of_foreign_revenue` spans ~1000x across companies, so a raw squared-error fit collapsed to an intercept-only model. Fixed by fitting in log-space and adding two internal-only features (`txn_count`, `n_countries`) derived from Syn Bank's own inbound transaction log |
...
- **The FX ElasticNet target originally showed no measurable signal** (R² negative across
  all 25 hyperparameter combinations) with a single raw-value feature and only 17 of 43
  company-years carrying a usable label. Fitting in log-space and adding two internal-only
  features (`txn_count`, `n_countries` from Syn Bank's own inbound transaction log) brought
  out-of-fold R² to ~0.38, in line with the other pillars.
